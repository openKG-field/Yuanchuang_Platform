  ...
  methods: {
    ...
    buildPlanPrompt() {
      const sections = [
        ...
        '必须包含章节：1. 总览与目标 2. 架构图文字描述 3. 模块职责表(表格) 4. 数据与接口设计 5. 核心算法/逻辑 6. 逐步实施里程碑与迭代计划 7. 关键代码骨架(核心环节) 8. 测试策略 9. 部署与运维 10. 安全 & 性能 11. 风险与回滚策略。',
        '特别要求：代码部分必须是最终方案中的核心环节（如核心算法、关键业务流），用于证明方案可行性。如果是伪代码，逻辑必须清晰完备。',
        '特别要求：每段代码必须包含“预期运行结果”或“输出示例”（Output Example），以展示代码执行后的效果。',
